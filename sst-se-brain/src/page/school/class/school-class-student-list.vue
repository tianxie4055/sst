<template>
	<swiper :options="swiperOption" class="list-swiper" ref="schoolClassStudentListSwiper" v-if="null!=list && list.length>0">
				...
				<li v-for="(dataItem,dataIndex) in item.items" :key="dataIndex">
					<a href="javascript:;" class="informationBox teacher">
						...
						<div class="objectHeadBox">
							...
							<div class="headECharts" :id="chartPrefix + index + '_' + dataIndex"></div>
							<div class="headData">
								<ul>
							...
							<div class="headName">
								<h1>{{dataItem.studentName}}</h1>
								<h2>{{dataItem.className}}</h2>
							</div>
						</div>
						<div class="introduce">
							<div class="overflow3" v-html="dataItem.studentDesc"></div>
						</div>
					</a>
				</li>
			</ul>
		</swiper-slide>
		<div class="swiper-pagination class-student-list-pager" slot="pagination"></div>
	...
	export default {
		data() {
			...
			return {
				...
				chartPrefix: 'chart_class_student_',
				swiperOption: {
					observer: true,
					observeParents: true,
					pagination: {
						el: '.class-student-list-pager',
						clickable: true,
		...
		methods: {
			...
			fetchData() {
				...
				this.loading = true;
				this.$fetch(this.$api.url.school.classStudentList, {
		...
		computed: {
			swiper() {
				return this.$refs.schoolClassStudentListSwiper.swiper;
			}
		},
		mounted() {
			this.fetchData();
