(function (factory) {
	if (typeof define === 'function' && define.amd) {
	    define(['echarts', 'jquery'], factory);
	} else {
	    factory(echarts, jquery);
	}
}(function (echarts, jquery) {
	...
	/**
	   绘制气泡图，data为气泡显示数据
	*/
	function renderBubbleChart(data, container, option) {
					
		var chart = echarts.init(container);
											   
		drawChart(chart, processBubbleData(data, container), option); 
	...
	*/
	function drawChart(chart, bubbleData, option) {		
		var defaultOption = {
			 tooltip: {
				show: true,
				triggerOn: 'none',
				formatter: function (params) {
					...
					var fontSize = textStyle.fontSize || 14;
					return '<span  style="border-radius: 6px;width: 12px;height: 12px;display: inline-block;background-color:' + params.color +';font-size:' + fontSize + 'px;"></span>' + ' ' +  params.data.name + ':  ' + params.data.value;
				}
			},
			graphic: getCircleOption(bubbleData, chart, option),
			series: []
		};
		var opts = jquery.extend(true, {}, defaultOption, option);
	...
	/**
	   ...
	   * @return {Array}  配置项集合
	*/			
	function getCircleOption(processData, chart, option) {
		 return echarts.util.map(processData, function (item, dataIndex) {
			return {
				...
				option: option,
				data: item.data,	
				children: getChildOption(item, dataIndex, chart, option),							
	...
	*/
	function getChildOption(item, dataIndex, chart, option) {
		var children = [];
		children.push({
			id: 'circle' + dataIndex,
			type: 'circle',
		...
		var text = item.data.name.length > 4 ? item.data.name.substr(0, 4) : item.data.name;
		var textStyle = jquery.extend({}, {
			fill: '#fff',
			text: '    ' + text,
			textAlign: 'center',
	...
	function showTooltip(event, chart) {
		var tooltipOpts = this.option.tooltip || {};
		var textStyle = tooltipOpts.textStyle || {};
